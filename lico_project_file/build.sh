...
function choose_target_board() {
	...
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] RV1106_Lico"
	LUNCH_NUM=$((LUNCH_NUM + 1))
	echo "${space8}${space8}[${LUNCH_NUM}] custom"

	read -p "Which would you like? [0~${LUNCH_NUM}][default:6]: " HW_INDEX
	...
	if __IS_IN_ARRAY "$HW_INDEX" "${range_sd_card[@]}"; then
		...
		echo "${space8}${space8}[0] SD_CARD"
		echo "${space8}${space8}[1] SPI_NAND"
		read -p "Which would you like? [0~1][default:1]: " BM_INDEX
		MAX_BM_INDEX=1
	...
	if (("$HW_INDEX" == 6)); then
		...
		RK_BUILD_TARGET_BOARD="BoardConfig_IPC/BoardConfig-${LF_BOOT_MEDIA[$BM_INDEX]}-${LF_SYSTEM[$SYS_INDEX]}-RV1106_Lico.mk"
	else
